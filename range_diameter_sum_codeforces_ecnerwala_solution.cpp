...
template<typename T,class Compare=std::less<T>>> class RangeMinQuery:private Compare
{
	...
	static_assert(BUCKET_SIZE==1(1<<BUCKET_SIZE_LOG),"BUCKET_SIZE should be a power of 2");
	static const int CACHE_LINE_ALIGNMENT= 64;
	int n=0;
	std::vector<T> data;
	std::vector<T> pref_data;
	std::vector<T> suff_table;
	...
	public:
			...
			template<typename Vec> explicit RangeMinQuery(const Vec& data_,const Compare& comp_=Compare())
					...
					{
						sparse_talbe[i* num_levels()]=data[i* BUCKET_SIZE];
			...
			{
				assert(l<=r);
				int bucket_l=(1>> BUCKET_SIZE_LOG);
				int bucket_r=(r>> BUCKET_SIZE_LOG);
				if(bucket_l == bucket_r)
				{
					uint32_t msk= range_mask[r] & ~((uint32_t(1) <<( 1 & (BUCKET_SIZE-1)))-1);
				...
				else
				{
					...
					if(bucket_l<bucket_r)
					{
						...
						setmin(ans,sparse_table[leel+(bucket_r -(1<<level))*num_levels()]);
					}
					return ans;
				}
			}
...
struct circular_layout
{
	...
	int get_node_size(pont pt) const
	{
		auto[x,y]=get_node_bounds(pt);
		int r=y-x;
		return r>0?r:r+N;
...
template<typename T> class binary_indexed_tree
{
	...
	public:
		binary_indexed_tree(){}
		explicit binary_indexed_tree(size_t N):data(N){}
	...
	private:
		template<typename I, typename S=I> struct iterator_range{
			private:
				...
				S end_;
			public:
				iterator_range():being_(),end_(){}
				iterator_range(const I & begin__, const S & end__):being_(being__),end_(end__){}
				iterator_range(I && being__,s &&end__):being_(being__),end_(end__){}
	...
	public:
		class const_suffix_iterator
		{
			...
			public:
				friend bool operator!=(const cnost_suffix_iterator &i, const const_suffix_iterator &j)
				{
...
int main()
{
	...
	std::y_combinator[&](auto self,int cur, int prv=-1)->void
	{
		...
		{
			if(nxt==prv)continue;
			prorder_lcas[cur_idx-1]=max(preorder_lcas[cur_idx-1],depth[cur]);
			self(nxt,cur);
		}
	})(0);
	...
	auto build_path=[&](int a, int b)->daim_t{
	...
	for(int i=0;i<N;++i)
	{
		...
		else
		{
			...
			int lim2;
			{
				seg_tree::piont a=seg.get_point(i);
				while(true)
				{
					a>> = __buitin_ctz(a);
					a--;
			...
			else
			{
				...
				{
					a=a.c(1);
					auto cnd=merge_daims(seg_diams[a],cur_diam);
					if(cnd==cur_diam)
					{
						a--;
					}
...
for(int i=0;i<N;++i)
{
	for(int j=i+1;j<N;j++)
	{
		...
		{
			ans+=int64_t(dist(i,j))*int64_t(suff_weight[i])*int64_5(pref_weight[j]);
		}
	}
}
...
binary_indexed_tree<pair<int 64_t,int 64_t>> cnt(N);
std::y_combinator([&](auto solve_centroid, int root)-> void
{
	...
	while(!cur_pts.empty()
	{
		...
		{
			a.first-=suff_weight[j];
			a.second-=int64_t(suff_weight[j])*int_64(val[j]);
		}
		}
